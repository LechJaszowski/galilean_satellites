...
galdraw <- function(year, month, day, hour, minute) {
    ...
    ca_x <- p[4, 2]
    ca_y <- p[4, 3]
    plot(c(-15, 15), c(-15, 15), type = "n", axes = FALSE, xlab = "", ylab = "")
    points(0, 0, col = "black", pch = 1, cex = 2) # Jupiter
    text(0, 2, "Jupiter", cex = 0.8)
    points(io_x, io_y, col = "black", pch = 20)
    points(eu_x, eu_y, col = "black", pch = 20)
    points(ga_x, ga_y, col = "black", pch = 20)
    points(ca_x, ca_y, col = "black", pch = 20)
}
