...
delta_t <- function(year, month) {
  ...
  for (i in seq_along(y)) {
    yi <- y[i]
    if (yi < -500) {
      deltat <- -20 + 32 * ((yi - 1820) / 100)^2
    } else if (yi < 500) {
      deltat <- 10583.6 - 1014.41 * (yi / 100) + 33.78311 * (yi / 100)^2 -
        5.952053 * (yi / 100)^3 - 0.1798452 * (yi / 100)^4 +
        0.022174192 * (yi / 100)^5 + 0.0090316521 * (yi / 100)^6
    } else if (yi < 1600) {
      deltat <- 1574.2 - 556.01 * ((yi - 1000) / 100) +
        71.23472 * ((yi - 1000) / 100)^2 + 0.319781 * ((yi - 1000) / 100)^3 -
        0.8503463 * ((yi - 1000) / 100)^4 - 0.005050998 * ((yi - 1000) / 100)^5 +
        0.0083572073 * ((yi - 1000) / 100)^6
    } else if (yi < 1700) {
      deltat <- 120 - 0.9808 * (yi - 1600) - 0.01532 * (yi - 1600)^2 +
        (yi - 1600)^3 / 7129
    } else if (yi < 1800) {
      deltat <- 8.83 + 0.1603 * (yi - 1700) - 0.0059285 * (yi - 1700)^2 +
        0.00013336 * (yi - 1700)^3 - (yi - 1700)^4 / 1174000
    } else if (yi < 1860) {
      deltat <- 13.72 - 0.332447 * (yi - 1800) + 0.0068612 * (yi - 1800)^2 +
        0.0041116 * (yi - 1800)^3 - 0.00037436 * (yi - 1800)^4 +
        0.0000121272 * (yi - 1800)^5 - 0.0000001699 * (yi - 1800)^6 +
        0.000000000875 * (yi - 1800)^7
    } else if (yi < 1900) {
      deltat <- 7.62 + 0.5737 * (yi - 1860) - 0.251754 * (yi - 1860)^2 +
        0.01680668 * (yi - 1860)^3 - 0.0004473624 * (yi - 1860)^4 +
        (yi - 1860)^5 / 233174
    } else if (yi < 1920) {
      deltat <- -2.79 + 1.494119 * (yi - 1900) - 0.0598939 * (yi - 1900)^2 +
        0.0061966 * (yi - 1900)^3 - 0.000197 * (yi - 1900)^4
    } else if (yi < 1941) {
      deltat <- 21.20 + 0.84493 * (yi - 1920) - 0.076100 * (yi - 1920)^2 +
        0.0020936 * (yi - 1920)^3
    } else if (yi < 1961) {
      deltat <- 29.07 + 0.407 * (yi - 1950) - (yi - 1950)^2 / 233 +
        (yi - 1950)^3 / 2547
    } else if (yi < 1986) {
      deltat <- 45.45 + 1.067 * (yi - 1975) - (yi - 1975)^2 / 260 -
        (yi - 1975)^3 / 718
    } else if (yi < 2005) {
      deltat <- 63.86 + 0.3345 * (yi - 2000) - 0.060374 * (yi - 2000)^2 +
        0.0017275 * (yi - 2000)^3 + 0.000651814 * (yi - 2000)^4 +
        0.00002373599 * (yi - 2000)^5
    } else if (yi < 2050) {
      deltat <- 62.92 + 0.32217 * (yi - 2000) + 0.005589 * (yi - 2000)^2
    } else if (yi < 2150) {
      deltat <- -20 + 32 * ((yi - 1820) / 100)^2 - 0.5628 * (2150 - yi)
    } else {
      deltat <- -20 + 32 * ((yi - 1820) / 100)^2
    }
  }
